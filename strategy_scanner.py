...
class StrategyScanner:
    ...
    def _load_all_patterns(self) -> None:
        ...
        if not os.path.exists(self.db_path):
            logger.info("[StrategyScanner] No se encontró DB. Usando seed_patterns.")
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # ✅ COMPROBACIÓN: Ver si la tabla existe antes de consultar
                cursor = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='patterns'")
                if cursor.fetchone() is None:
                    logger.info("[StrategyScanner] Tabla 'patterns' no existe. Usando seed_patterns.")
                    for tf in self.timeframes:
                        self.patterns_by_tf[tf] = [p.copy() for p in seed_patterns.PATTERNS if p.get("timeframe") == tf]
                    return
